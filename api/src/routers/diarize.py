...
async def diarize_endpoint(video_id: str):
    ...
    video_path = settings.videos_dir / f"{title}.mp4"
    audio_path = diar_dir / f"{title}.wav"
    subprocess.run(["ffmpeg", "-i", str(video_path), "-vn", "-acodec", "pcm_s16le", "-ar", "16000", "-y", str(audio_path)], check=True)
    ...
    # Merge speaker labels into transcription JSON for downstream stages
